...
const ChatBox = forwardRef(({ context, onChartUpdate, chatState, setChatState }, ref) => {
  ...
  const handleSendMessage = async (e, customMessage = null) => {
    ...
    try {
      ...
      if (result.type === 'chart_update') {
        ...
        addBotMessage("I've updated the chart based on your request. How does it look?");
      } else {
      ...
      <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-200">
        <div className="flex gap-2">
          <input
            type="text"
            value={inputMessage}
            onChange={(e) => updateChatState({ inputMessage: e.target.value })}
